...
<div class="max-w-3xl mx-auto px-4 sm:px-6 md:px-10 py-8 md:py-10">
    ...
    <div class="bg-white rounded-2xl shadow-sm border p-6 sm:p-8 mb-6 sm:mb-8">
        ...
        <form method="POST" action="{{ route('settings.update') }}" data-no-loader class="space-y-6">
            ...
            @elseif (auth()->user()->isEmployer())
                <label class="flex items-start gap-3">
                    <input type="checkbox" name="notify_new_applications" value="1"
                           @checked(auth()->user()->notify_new_applications)
                           class="mt-1 rounded border-gray-300 text-[#1E3A6D] focus:ring-[#1E3A6D]">
                    <span>
                        <span class="block text-sm font-semibold text-slate-800">New applications</span>
                        <span class="block text-sm text-slate-500">Email me when a candidate applies to one of my jobs.</span>
                    </span>
                </label>
            @else
                <p class="text-sm text-slate-500">
                    No email notification preferences are available for this account type.
                </p>
            @endif

            <div class="pt-2">
                <button type="submit"
                        class="inline-flex w-full sm:w-auto items-center justify-center gap-2 px-6 py-3 rounded-xl bg-[#1E3A6D] text-white
    ...
    <div class="bg-white rounded-2xl shadow-sm border p-6 sm:p-8">
        ...
        <a href="{{ route('profile.edit') }}"
           class="inline-flex w-full sm:w-auto items-center justify-center gap-2 px-6 py-3 rounded-xl border border-slate-300
